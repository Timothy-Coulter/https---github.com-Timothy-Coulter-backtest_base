...
def drop_database():
    ...
    response = input(f"⚠️  Are you sure you want to DROP database '{db_name}'? This will delete ALL studies! (yes/no): ")
...
def list_studies():
    ...
    try:
        ...
        for study_summary in studies:
            best_value = f"{study_summary.best_trial.value:.4f}" if study_summary.best_trial else "N/A"
            print(f"{study_summary.study_name:<30} {study_summary.direction.name:<12} {study_summary.n_trials:<10} {best_value:<15}")
...
def create_study(study_name, direction="maximize"):
    ...
    try:
        ...
        print(f"✗ Study '{study_name}' already exists. Use a different name or delete the existing study first.")
    except Exception as e:
...
Examples:
  ...
  python src/db_manager.py --recreate-db
    ...
    parser.add_argument("--recreate-db", action="store_true", help="Drop and recreate the database (destructive!)")
