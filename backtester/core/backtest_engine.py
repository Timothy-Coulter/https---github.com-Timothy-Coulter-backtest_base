...
class BacktestEngine:
    ...
    def __init__(self,
        ...
        self.data_handler = DataHandler(self.config.__dict__ if hasattr(self.config, '__dict__') else None, self.logger)
        ...
        self.portfolio: Optional[DualPoolPortfolio] = None
        ...
        self.current_portfolio: Optional[DualPoolPortfolio] = None
        self.current_broker: Optional[SimulatedBroker] = None
        self.current_risk_manager: Optional[RiskManager] = None
    ...
    def load_data(self, ticker: Optional[str] = None, start_date: Optional[str] = None,
                  end_date: Optional[str] = None, interval: Optional[str] = None) -> pd.DataFrame:
        ...
        start_date = start_date or (self.config.data.start_date if self.config.data else "2015-01-01")
    ...
    def create_strategy(self, strategy_params: Optional[Dict[str, Any]] = None) -> BaseStrategy:
    ...
    def create_portfolio(self, portfolio_params: Optional[Dict[str, Any]] = None) -> DualPoolPortfolio:
        ...
        self.logger.info(f"Created portfolio with ${self.current_portfolio.initial_capital:.2f} initial capital")
    ...
    def create_broker(self) -> SimulatedBroker:
        ...
        self.current_broker = SimulatedBroker(
            commission_rate=self.config.execution.commission_rate if self.config.execution else 0.001,
            ...
            slippage_model=self.config.execution.slippage_model if self.config.execution else "normal",
        ...
        if self.current_data is not None:
            assert self.config.data is not None
            self.current_broker.set_market_data(
                self.config.data.default_ticker,
                self.current_data
            )
    ...
    def create_risk_manager(self) -> RiskManager:
        ...
        self.current_risk_manager = RiskManager(
            ...
            alert_thresholds={
                'var_threshold': self.config.risk.max_portfolio_risk * 1.2 if self.config.risk else 0.024,
                'drawdown_threshold': self.config.risk.max_drawdown * 1.1 if self.config.risk else 0.22,
    ...
    def run_backtest(self, ticker: Optional[str] = None, start_date: Optional[str] = None,
                    end_date: Optional[str] = None, interval: Optional[str] = None,
                    strategy_params: Optional[Dict[str, Any]] = None,
                    portfolio_params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def _process_signals_and_update_portfolio(self, signals: List[Dict[str, Any]], 
                                            current_price: float, day_high: float, 
                                            day_low: float, timestamp: Any) -> Dict[str, Any]:
    ...
    def _check_risk_management(self, portfolio_value: float) -> None:
        ...
        assert self.current_risk_manager is not None
        risk_signal = self.current_risk_manager.check_portfolio_risk(portfolio_value, positions_dict)
        
        if risk_signal.action.value in ['REDUCE_POSITION', 'CLOSE_POSITION']:
            # Cancel existing orders or close positions
            self.current_broker.order_manager.cancel_all_orders(f"Risk management: {risk_signal.reason}")
    ...
    def _calculate_performance_metrics(self) -> Dict[str, Any]:
        ...
        if self.config.performance.benchmark_enabled:
            try:
                ...
                benchmark_values = benchmark_data['Close'] * (portfolio_values.iloc[0] / benchmark_data['Close'].iloc[0])
            except Exception as e:
    ...
    def generate_performance_report(self) -> str:
        ...
        assert self.current_data is not None
        report.append(f"Data Period: {self.current_data.index[0].strftime('%Y-%m-%d')} to {self.current_data.index[-1].strftime('%Y-%m-%d')}")
    ...
    def plot_results(self, save_path: Optional[str] = None, show_plot: bool = True):
    ...
    def _run_strategy_backtest(self, strategy_params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def _validate_config(self) -> bool:
        ...
        try:
            ...
            if self.config.portfolio and hasattr(self.config.portfolio, 'initial_capital'):
                try:
                    if self.config.portfolio.initial_capital <= 0:
                        return False
            ...
            if self.config.strategy:
                try:
                    if hasattr(self.config.strategy, 'leverage_base') and self.config.strategy.leverage_base <= 0:
                        return False
                    if hasattr(self.config.strategy, 'leverage_alpha') and self.config.strategy.leverage_alpha <= 0:
                        return False
                except (TypeError, AttributeError):
                    pass  # Skip validation for missing or non-numeric attributes
            
            # Check required configs exist (but don't assert to avoid exceptions)
            if (hasattr(self.config, 'data') and self.config.data is not None and
                hasattr(self.config, 'strategy') and self.config.strategy is not None and
                hasattr(self.config, 'portfolio') and self.config.portfolio is not None):
                return True
    ...
    def _calculate_performance(self, trades: pd.DataFrame, portfolio_values: pd.Series) -> Dict[str, Any]:
        ...
        if hasattr(self, 'current_portfolio') and self.current_portfolio is not None:
            try:
                metrics.update({
                    'total_trades': len(self.current_portfolio.trade_log) if hasattr(self.current_portfolio, 'trade_log') else len(trades),
                    'cumulative_tax': getattr(self.current_portfolio, 'cumulative_tax', 0.0),
                    'base_pool_final': getattr(self.current_portfolio.base_pool, 'capital', 0.0) if hasattr(self.current_portfolio, 'base_pool') else 0.0,
                    'alpha_pool_final': getattr(self.current_portfolio.alpha_pool, 'capital', 0.0) if hasattr(self.current_portfolio, 'alpha_pool') else 0.0
                })
            except Exception:
