...
from backtester.data.data_handler import get_data, DataHandler
...
if _run_sim_available:
    run_portfolio_simulation: Union[Callable[..., Any], Any] = _original_run_sim
    src_get_data: Union[Callable[..., Any], Any] = _original_get_data_util
else:
    run_portfolio_simulation = None  # type: ignore[assignment]
    src_get_data = None  # type: ignore[assignment]
...
def compatibility_objective(trial: Any, data: pd.DataFrame) -> float:
    ...
    penalty = 50
    score = result["performance"]["total_return"] - penalty * abs(result["performance"]["max_drawdown"] * 100)
    return score
...
def compare_systems(data: pd.DataFrame, params: Dict[str, float]) -> Dict[str, Any]:
    ...
    comparison = {
        ...
        'modular': {
            'total_return': modular_result['performance']['total_return'] * 100,  # Convert to percentage
            'max_drawdown': modular_result['performance']['max_drawdown'] * 100,  # Convert to percentage
    ...
    comparison['differences'] = {
        'total_return_diff': comparison['original']['total_return'] - comparison['modular']['total_return'],
        'max_drawdown_diff': comparison['original']['max_drawdown'] - comparison['modular']['max_drawdown'],
        'sharpe_ratio_diff': comparison['original']['sharpe_ratio'] - comparison['modular']['sharpe_ratio'],
        'final_value_diff': comparison['original']['final_value'] - comparison['modular']['final_value']
    }
...
def main():
    ...
    parser.add_argument('--mode', choices=['single', 'optimize', 'multi', 'compare'], 
    ...
    if args.mode == 'single':
        ...
        print(f"Best Total Return: {optimization_result['best_result']['performance']['total_return']:.2%}")
        print(f"Best Sharpe Ratio: {optimization_result['best_result']['performance']['sharpe_ratio']:.3f}")
        ...
        for ticker, result in multi_result['individual_results'].items():
            print(f"{ticker}: {result['performance']['total_return']:.2%} return, "
...
class ModularBacktester:
    ...
    def run_backtest(self, data: pd.DataFrame, strategy: Any,
                    portfolio: Any, broker: Any, **kwargs) -> Dict[str, Any]:
...
def run_backtest(symbol: str, start_date: str, end_date: str,
                interval: str = "1d", config: Optional[Dict] = None) -> Dict[str, Any]:
...
def optimize_parameters(param_ranges: Dict[str, Any], objective_function: Any,
                       method: str = "grid_search", max_iterations: int = 100) -> Dict[str, Any]:
...
def run_optimization(data: pd.DataFrame, param_ranges: Dict[str, Any],
                    objective_function: Any, optimization_method: str = "grid_search") -> Dict[str, Any]:
...
class BacktesterApp:
    ...
    def run_single_backtest(self, symbol: str, data: pd.DataFrame, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            Backtest results
        """
        strategy = self.strategy_factory.create('moving_average', config)
        portfolio = self.portfolio_factory.create('general', config)
    ...
    def run_parameter_optimization(self, data: pd.DataFrame, param_ranges: Dict[str, Any],
    ...
    def run_batch_backtests(self, symbols: List[str], data_dict: Dict[str, pd.DataFrame],
    ...
    def generate_report(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        report = {
            ...
            'trade_summary': result.get('trades', pd.DataFrame()).to_dict('records') if 'trades' in result else [],
    ...
    def validate_configuration(self, config: Dict[str, Any]) -> tuple[bool, List[str]]:
        ...
        if 'commission_rate' in config:
            if not isinstance(config['commission_rate'], (int, float)) or config['commission_rate'] < 0:
                errors.append("Commission rate must be a non-negative number")
        
        return len(errors) == 0, errors
    
    def configure_logging(self, level: str = "INFO", log_file: Optional[str] = None) -> None:
