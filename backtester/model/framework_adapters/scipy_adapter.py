...
class SciPyAdapter(ModelFrameworkAdapter):
    ...
    def predict(self, model: Callable[..., Any], features: pd.DataFrame) -> np.ndarray:
    ...
    def evaluate_model(
        self, features: pd.DataFrame, target: pd.Series, predictions: np.ndarray
    ) -> dict[str, float]:
