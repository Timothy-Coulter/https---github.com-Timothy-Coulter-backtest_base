...
class SklearnAdapter(ModelFrameworkAdapter):
    ...
    def initialize_model(self, config: SklearnModelConfig) -> Any:
    ...
    def predict_proba(self, model: BaseEstimator, features: pd.DataFrame) -> np.ndarray:
    ...
    def get_feature_importance(self, model: Any) -> np.ndarray | None:
        ...
        if hasattr(model, 'feature_importances_'):
            return model.feature_importances_  # type: ignore[no-any-return]
        elif hasattr(model, 'coef_'):
            return np.abs(model.coef_)  # type: ignore[no-any-return]
        else:
            return None

    def get_model_params(self, model: Any) -> dict[str, Any]:
