...
class ValidationUtils:
    ...
    def validate_config(self, config: Dict[str, Any]) -> bool:
        ...
        if 'initial_capital' in config:
            if not self.validate_numeric(config['initial_capital']) or config['initial_capital'] <= 0:
                return False
        ...
        for field in numeric_fields:
            if field in config and config[field] is not None:
                ...
                if field == 'leverage' and config[field] <= 0:
                    return False
                if field == 'max_positions' and (config[field] < 0 or not isinstance(config[field], int)):
                    return False
        
        return True
    
    def validate_range(self, value: Union[int, float], min_value: float = float('-inf'), 
