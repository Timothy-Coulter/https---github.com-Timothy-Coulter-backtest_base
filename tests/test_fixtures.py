...
def mock_strategy():
    ...
    strategy = Mock()
    strategy.generate_signals = Mock(return_value=pd.Series([0, 1, 0, -1, 1], 
                                                           index=pd.date_range('2020-01-01', periods=5)))
    strategy.validate_parameters = Mock(return_value=True)
    strategy.optimize_parameters = Mock(return_value={
...
def mock_portfolio():
    ...
    portfolio.get_allocation = Mock(return_value={
        'SPY': 0.40, 'AAPL': 0.25, 'CASH': 0.35
    })
