...
try:
    from backtester.core.performance import (
        PerformanceCalculator, PerformanceMetrics, RiskMetrics,
        DrawdownAnalyzer, ReturnAnalyzer, SharpeCalculator
    )
...
class TestReturnAnalyzer:
    ...
    def test_calculate_rolling_returns(self):
        ...
        assert result.iloc[window-1:].notna().all()  # Values from window onwards should be calculated
